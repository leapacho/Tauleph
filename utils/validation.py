...
async def validate_message(message: discord.Message) -> bool:
    ...
    bot_name_in_message = discord_obj.bot_name.lower() in message.content.lower() # True if the bot's name is in the message and it comes from an allowed channel.
    bot_mentioned = discord_obj.bot_user in message.mentions # True if the bot is mentioned in the message and it comes from an allowed channel.
